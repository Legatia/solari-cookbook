...
**Implementation status:** the first-party OAuth connection, PKCE exchange, session-bound agent bootstrap, discoverable-passkey login, second-browser recovery of the canonical agent, context recall, visible connection management, revocation, high-level memory/research/introduction actions, leases, handoff, consent, entitlement refusal, lost-passkey recovery codes, verified-webhook credit grants, and scheduler health are implemented and exercised against Neon by `pnpm verify:phase-one`. A real internal-model invocation with project credentials is verified against `gpt-5.6-luna`. Credential removal/renaming and a fully exercised second-device ceremony remain before the full gate is complete.
...
- Detect password, one-time-code, and payment controls and stop at a human checkpoint. Issue a thirty-minute single-use Solari handoff only for the participant's approved mission, current browser profile, and approved origin; rehearse real password, OTP, payment, and cross-origin OAuth variants before claiming general support.
...
- Solari's commercial terms for multi-tenant brokerage and production pricing for Desktop, volumes, and snapshots, including snapshot storage beyond the 10 GB free allowance from October
