...
This is not the full Phase 1 gate: lost-passkey recovery, a real billing provider and verified webhook, production scheduler monitoring, and a real internal-model invocation with project credentials still remain. Solari snapshot storage becomes billable in October; Sylla now prunes the snapshot each checkpoint and pause replaces, which is verified against the live API, but a retention policy for long-lived agents is still an open decision.
