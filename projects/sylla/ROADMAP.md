...
The portability foundation is also underway: the database assigns canonical Sylla User and Personal Agent identifiers independently of host providers, records Desktop/volume/snapshot lifecycle metadata, and exposes a stateless Streamable HTTP MCP contract for agent bootstrap, approved-context recall, plan inspection, run leases, durable runs/checkpoints/handoffs, and workspace inspection/open/checkpoint/pause. Opening creates one durable volume, reconnects or resumes the existing Desktop when present, reconstructs the visible workbench only from approved state, and records a recovery snapshot; pausing checkpoints first and preserves the home for another client or the later native app. Exclusive leases bind the authenticated client plus conversation run ID to one agent, store only a capability hash, and support heartbeat, expiry, release, and cross-host handoff. The first fallback task is intentionally limited to a non-consequential reconnect summary: an atomic database claim refuses a live host lease, enforces the participant-approved budget, advances from the latest explicit checkpoint, and writes one auditable handoff exactly once even when eight workers race. A second host can acquire a new lease, read the handoff, and acknowledge it without duplicating the task. Trial/active entitlements guard billable work through atomic reservations and an idempotent usage ledger; inactive accounts receive an expiring hosted-checkout continuation with no payment data entering MCP. These behaviors have been race- and lifecycle-tested against Neon. This is not the full Phase 1 gate: a real identity-provider tenant, browser consent flow, participant-visible connection revocation, a real billing provider and verified webhook, an automatic fallback scheduler, and the bounded semantic internal-model adapter still remain.
