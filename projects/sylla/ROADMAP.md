...
The portability foundation is also underway: the database now assigns canonical Sylla User and Personal Agent identifiers independently of host providers, records Desktop/volume/snapshot lifecycle metadata, and exposes a stateless Streamable HTTP MCP contract for idempotent agent bootstrap, approved-context recall, and workspace inspection/open/checkpoint/pause. Opening creates one durable volume, reconnects or resumes the existing Desktop when present, reconstructs the visible workbench only from approved state, and records a recovery snapshot; pausing checkpoints first and preserves the home for another client or the later native app. The endpoint supports RFC 9728 discovery and validates external JWT access tokens against an exact issuer, audience, expiry, client identifier, and `sylla:agent` scope. A verified issuer/subject deterministically recovers the same user and agent across different client IDs, while host connections are recorded separately without persisting access tokens. This behavior has been exercised against Neon. This is not the full Phase 1 gate: a real identity-provider tenant, browser consent flow, participant-visible connection revocation, entitlements, and checkout continuation still remain.
