...
The portability foundation is also underway: the database assigns canonical Sylla User and Personal Agent identifiers independently of host providers, records Desktop/volume/snapshot lifecycle metadata, and exposes a stateless Streamable HTTP MCP contract for agent bootstrap, approved-context recall, plan inspection, run leases, durable runs/checkpoints/handoffs, and workspace inspection/open/checkpoint/pause. Opening creates one durable volume, reconnects or resumes the existing Desktop when present, reconstructs the visible workbench only from approved state, and records a recovery snapshot; pausing checkpoints first and preserves the home for another client or the later native app. Exclusive leases bind the authenticated client plus conversation run ID to one agent, store only a capability hash, and support heartbeat, expiry, release, and cross-host handoff. The first fallback task is intentionally limited to a non-consequential reconnect summary. An authenticated scheduled sweep acquires the same runtime lease as host clients, claims the participant-approved budget, invokes a bounded provider-neutral adapter, writes one auditable handoff, and releases. The deterministic default makes no external call; optional live mode sends only the explicit checkpoint through a strict-schema, non-stored, token- and time-bounded Responses request, then degrades safely on provider failure. Eight-worker races, returning-host overlap, model failure, and stale-worker recovery without a second charge have been exercised against Neon. Trial/active entitlements guard billable work through atomic reservations and an idempotent usage ledger; inactive accounts receive an expiring hosted-checkout continuation with no payment data entering MCP. This is not the full Phase 1 gate: a real identity-provider tenant, browser consent flow, participant-visible connection revocation, a real billing provider and verified webhook, production scheduler monitoring, and a real internal-model invocation with project credentials still remain.
