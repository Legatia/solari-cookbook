...
The portability promise is operational, not branding: approved memory must resolve across every participation record linked to the canonical agent, appear in a versioned machine-readable export, and rebuild the same Desktop workbench in a later host or native app. Full deletion requires an explicit human-host confirmation, attempts to destroy every Sylla-managed workspace, and then removes the canonical account plus its linked participation state. Exports and organizer evidence must omit raw debriefs, provider credentials, Desktop capabilities, and other participants' private outcomes.
