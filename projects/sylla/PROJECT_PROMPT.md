...
The web application is the durable trust and control center: identity, proposed and approved memory, evidence and provenance, permissions, connected AI hosts, introductions, private workspace access, fallback status, export, and deletion. It must let the participant inspect everything Sylla holds about them and directly Keep, Correct, make Private/Shareable, or Forget both evidence observations and distilled relationship memories. It may provide a reference conversation for demos and recovery, but it should not require participants to abandon their existing AI habit.

Sylla—not any host LLM and not a Solari VM—owns the canonical user and agent identity. The first MCP authorization creates or links a permanent Sylla account and a permanent user-owned agent record. From that trusted session, the participant can enroll a discoverable passkey and later recover the same account through a fresh browser session without using the LLM provider as Sylla's identity authority. ChatGPT, Claude, Gemini, Grok, the web application, and a future native Sylla application are interchangeable clients of that same agent. A participant who later signs into the native application with a linked identity must recover the same approved memory, relationships, permissions, subscription, work history, and private workspace without migration or reset.
...
Treat MCP as the conversational control surface for zero-infrastructure onboarding. After Sylla OAuth, an idempotent bootstrap operation creates or links the agent and prepares its durable workspace metadata. The MCP setup guide must return a conversation contract, not merely missing fields: begin with what would make the agent worth keeping, ask at most one question per reply, reflect specific answers briefly, collect the participant's chosen names, then explain and explicitly confirm the trust boundaries. Matching and background continuation are optional; ask for availability only when matching is enabled. Never lead with a permission dump or generic AI enthusiasm. Return the standalone web setup as a fallback for participants who prefer a form or visual review, not as the default path. Sylla holds its own server-side Solari credentials and provisions Browser, Sandbox, Desktop, volumes, and snapshots on the participant's behalf. The participant must never need a Solari account or API key.

The portability promise is operational, not branding: approved memory must resolve across every participation record linked to the canonical agent, appear in a versioned machine-readable export, and rebuild the same Desktop workbench in a later host or native app. Full deletion requires an elevated OAuth scope, an explicit human-host confirmation, and an exact destructive phrase; it attempts to destroy every Sylla-managed workspace and then removes the canonical account plus its linked participation state. Exports and organizer evidence must omit raw debriefs, provider credentials, Desktop capabilities, and other participants' private outcomes.
...
Represent that boundary structurally in the runtime lease: host, web, and internal owners are distinct. Disclosure-envelope approval and introduction acceptance require a live human-controlled host lease, not merely any valid lease. Each participant approves one to five confirmed shareable observations for a recommended pair. Until both people accept, a proposal may show only the other person's approved anonymous preview; it must hide identity, meeting details, evaluation rationale, private observations, and the other person's response. A decline closes the proposal without revealing who declined. Mutual acceptance reveals only the participant names and preconfigured public meeting area needed for the meeting.
...
The first fallback milestone stays narrower than the eventual semantic agent: `prepare_reconnect_summary` requires the worker to acquire the same exclusive lease as host clients, charges at most one approved fallback credit, preserves the latest explicit checkpoint, and takes no consequential action. The scheduled controller bounds model input, timeout, output tokens, and schema; disables provider storage where supported; degrades to a deterministic handoff on model failure; recovers a stale worker without charging twice; and atomically commits the checkpoint plus reconnect handoff. The next implemented task type, `research_approved_sources`, may continue only the exact public URLs already stored in the run scope, one source per host call and one checkpoint per completed source. It must not widen scope or automatically revisit completed, failed, or ambiguous in-flight sources. These are bounded task-specific continuations, not authority for a general autonomous agent.
