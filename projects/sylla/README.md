...
Pricing is split along Sylla's own cost structure rather than by volume. The society — the agent, its memory, boundaries, invitations, introductions — is rows in Postgres and costs almost nothing per person, so **Resident** is free forever and is never withheld for non-payment. Solari compute has a real unit cost, so that is what a subscription buys: **Working** at $12 a month for 2,000 credits, **Deep** at $40 for 10,000. One-off packs remain alongside them, because a burst of work should not require a monthly commitment.

Unused credits carry over. Expiring them each month would recreate exactly the "I paid for a month I did not use" resentment the packs were chosen to avoid, and it costs nothing to carry: an unspent credit is compute that was never bought. Cancelling drops the tier to Resident and takes nothing away — credits already paid for were bought, not rented.

Credits are granted by the paid invoice and nowhere else. The first month arrives as both a completed checkout and a paid invoice, so paying out on both would hand over two months for one payment; the checkout only settles the Sylla row and links the subscription. The tier and user ride on the subscription's own metadata, so a renewal months later is self-describing and does not depend on the checkout webhook having arrived first, which Stripe does not promise. `pnpm verify:subscription` holds all of that, including that a retried invoice pays once.
...
Sylla can also reach out, but only if asked to. It holds no address by default — that is why recovery codes exist rather than a reset link — so a participant who wants to be told supplies one, and nothing is sent until they open a link proving it is theirs. Changing the address un-verifies it, so proving one address cannot be used to redirect the mail somewhere else.

One rule governs every message and is the reason this can exist at all: it says *that* something happened and never *what*. No claim, no name, no dossier line, no evidence — a count and a link to the control room. Mail lands on somebody else's server and stays there, and a product whose promise is that private material stays put cannot post that material to Gmail the moment it becomes interesting. The rest is restraint: at most one message a day, preferences per kind, and a one-click unsubscribe that needs no password and keeps working on messages sent months ago because the token is derived rather than stored. `pnpm verify:notifications` holds all nine of those.

The fallback sweep sends one message per participant when it finishes work they were away for, after the sweep and swallowing its own failures — a mail provider being down must never roll back work that already completed.

The control room also carries the receipt for unattended work. Sylla asks people to let an agent carry on after they have closed the chat, which is only reasonable to ask if they can afterwards see exactly what happened: every run, whether they were present, what it got done taken from its own checkpoints rather than restated from the request, what it cost, and which model stood in when one did. An agent that ran on something is not the same as an agent that ran, so the log names it.

Resting a workspace releases its machine rather than pausing it. Solari confirmed that a paused desktop keeps its concurrency slot, so pausing would have let one participant who opened a workspace and walked away hold a machine indefinitely while the next person was simply refused — on a plan with two desktops, that is the whole product for everybody else. The durable volume carries the state, so nothing is lost by letting the machine go: resting means the work is safe, not that a machine is idling. A snapshot is taken first, because Solari refuses one on a stopped machine, and a destroy that fails falls back to a pause so a failure can never leave compute running.

Waking it is therefore a third kind of operation, priced as one. A held machine only needs waking and costs little; a released one is a real provision against an existing volume and snapshot, so it costs most of an open but not the volume creation and first workbench build; a first open pays for everything. The participant pays for the one that actually happens.

The scheduled sweep runs its candidates in parallel rather than one after another, bounded by a setting rather than a constant. The bound is a fraction of the provider's concurrency, never all of it: a sweep that spends the whole allowance hands a 503 to whoever is awake and using Sylla at that moment. Browser research and reconnect summaries are bounded separately, because the first holds a real browser for its whole run and the second is a model call contending for a different limit entirely. Each worker pulls the next item rather than taking a fixed slice, so one slow source cannot leave the rest idle behind it.

Work done in a Solari Browser session is recorded, and the log offers a replay of it. That turns the account of an unattended run from something the agent wrote about itself into something a person can watch — which is the difference between an assurance and evidence. The link is minted when somebody presses watch rather than stored, because replay URLs are presigned and short lived and one saved in a list would be a dead link by the time anyone went looking; the log carries only whether a recording exists. A replay that is still processing is reported as such rather than as missing, since one is worth trying again shortly and the other never will be.

The headline is stated rather than inferred, because "nothing happened" and "I cannot tell" look identical in an empty list. `consequentialWhileAway: false` is an assertion that nothing irreversible happened unattended, and it is structural rather than lucky: the background worker writes `consequentialActionsTaken: false` by construction, since it summarises and does not act. Where a provider failed and the summary is Sylla's own fallback rather than the agent's account, the entry says so. `pnpm verify:work-log` drives a real run through host handoff into the background worker and checks each of those.

An agent also keeps a book on everyone else. A dossier is a record about a person or organization — an investor, a firm, a counterpart — with each claim carrying how it was learned: told to the agent by its own participant, read from an approved source, or worked out by the agent and therefore weakest. Dossiers are reachable from the control room and from the agent through `sylla_note_about` and `sylla_read_dossier`.

Dossiers carry a stage — not yet approached, talking, in diligence, committed, passed — and one set serves both sides of the table, because a founder tracking an investor and an investor tracking a founder walk the same funnel from opposite ends. A term sheet is the same event whoever writes it.

The control room leads with what is waiting on the participant rather than with a list, since the question someone running a raise opens this with is never "show me everyone" but "what have I dropped". Three things qualify and nothing is scored: a promise whose date has passed, a conversation that has gone quiet, and someone added a week ago and never approached. Silence is read against the stage — a fortnight without contact during diligence is a problem, the same fortnight after a decision is not — because a pipeline that cries wolf stops being read. The agent reads the same view through `sylla_read_dossier` and is told to lead with it.

This is the first record in Sylla about someone who is not present and did not consent, so it is fenced rather than merely stored. A dossier belongs to exactly one participant and is never pooled. Its claims are written private and cannot be made shareable, so they cannot enter a disclosure envelope, an introduction, or a matching evaluation — every query that reads observations for any of those purposes now also excludes them, including the one that feeds a participant's own private context into a Sandbox evaluation. They are absent from "what Sylla knows", which stays the participant's account of themselves. They leave in a portable export under their own key rather than mixed in with that account. And closing a dossier deletes what it held, because someone asking Sylla to forget a third party has asked for the strongest thing it can do. `pnpm verify:dossier` holds all of that.
